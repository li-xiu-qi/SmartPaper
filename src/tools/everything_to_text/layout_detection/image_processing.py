...
def sort_page_layout(rendered_image_path, output_dir, page_num):
    ...
    Args:
        ...
        output_dir: 输出目录
        page_num: 页码
        skip_layout_detection: 是否跳过版面分析，用于测试或在不需要版面分析的场景
...
def process_image_box(box, rendered_image_path, output_dir, image_store=None):
    """
    处理单个图像框 - 进行图像提取和保存，返回图像信息
    ...
    with open(file=new_image_path, mode="rb") as img_file:
        ...
        # image_key = image_name.split(".")[0]  # 使用文件名作为键名（不带扩展名）
        image_key = image_name
        image_store.save_image(key=image_key, base64_data=img_base64)
        print(f"图像已保存到存储: {image_key}")
        
    # 直接生成图像描述和标题
    description = describe_image(image_path=new_image_path)
    title = get_image_title(description)
...
def extract_images_from_layout(layout_result, rendered_image_path, output_dir, image_labels, image_store=None):
    """
    从布局结果中提取所有图像
    ...
    Args:
        ...
        image_labels: 图像标签集
        image_store: 图像存储对象
        pdf_name: PDF文件名，用于存储标识
        
    Returns:
        tuple: (图像Markdown文本, 图像数量, 图像信息列表)
    """
    image_markdown = ""
    image_infos = []  # 存储图像信息
    ...
    if "boxes" in layout_result:
        ...
        if boxes:
            ...
            for box in boxes:
                image_info = process_image_box(box=box,
